...
DEFINE_int64(num, 240000000,
             "Number of key/values to place in database");  // about 256 GB
...
class Stats {
 ...
 public:
  ...
  std::string ReportMemory(DB* db) {
    std::string result = "";

    std::string index_size;
    db->GetProperty("rocksdb.estimate-table-readers-mem", &index_size);
    result += (index_size + ",");
    std::string memtable_size;
    db->GetProperty("rocksdb.cur-size-all-mem-tables", &memtable_size);
    result += (memtable_size + ",");
    std::string sst_size;
    db->GetProperty("rocksdb.total-sst-files-size", &sst_size);
    result += (sst_size + ",");
    std::string block_usage;
    db->GetProperty("rocksdb.block-cache-usage", &block_usage);
    result += (block_usage + "/");
    std::string block_capacity;
    db->GetProperty("rocksdb.block-cache-capacity", &block_capacity);
    result += (block_capacity + ",");
    std::string pinned_usage;
    db->GetProperty("rocksdb.block-cache-pinned-usage", &pinned_usage);
    result += (pinned_usage + "\n");
    return result;
  }
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    // DbPath hdd = DbPath("/media/supermt/hdd/dataset", 200l * 1024 * 1024 *
    // 1024); if (options.db_paths.size()!=1){
