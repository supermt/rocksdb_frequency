...
namespace rocksdb {
  ...
  namespace {
    uint64_t
    TotalCompensatedFileSize(const std::vector<FileMetaData *> &files) {
      uint64_t sum = 0;
...
// Delete this compaction from the list of running compactions.
  ...
  void
  CompactionPicker::GetRange(const std::vector<CompactionInputFiles> &inputs,
                             InternalKey *smallest,
                             InternalKey *largest) const {
  ...
  Compaction *CompactionPicker::CompactFiles(
    ...
    if (compact_options.compression == kDisableCompressionOption) {
      ...
      if (ioptions_.compaction_style == kCompactionStyleLevel &&
          ioptions_.compaction_style == kCompactionStyleQuickSand) {
...
// Test whether two files have overlapping key-ranges.
    ...
    for (int l = 0; l <= output_level; ++l) {
      ...
      if (l == 0) {
        for (int f = first_included; f <= last_included; ++f) {
          ...
          if (comparator->Compare(largestkey, current_files[f].largestkey) <
              0) {
            largestkey = current_files[f].largestkey;
          }
        }
  ...
  void CompactionPicker::PickFilesMarkedForCompaction(
    ...
    auto continuation = [&, cf_name](
      std::pair<int, FileMetaData *> level_file) {
